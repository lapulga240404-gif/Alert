...
async function checkBlinkitWithBrowser(productUrl) {
  ...
  try {
    ...
    const result = await page.evaluate(() => {
      // Look for Add button - try multiple patterns
      ...
      // More specific out of stock detection - only check main content
      const mainContent = document.querySelector('main') || document.body;
      const pageText = mainContent.innerText.toLowerCase();
      
      // Look for specific out of stock phrases
      ...
      // Check if "Notify Me" button exists
